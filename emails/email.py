...
class EmailManager:
    ...
    def sendAlertEmails(items):
        ...
        if len(emails) == 0 or len(items) == 0:
            return
        missingImage = f"https://{settings.EMAIL_DOMAIN}/static/itemManagement/default_image.svg"
        items = list(map(lambda item: {'name': item.title,
                                       ...
                                       "image": missingImage \
                                           ...
                                           signer.sign(f"https://{settings.EMAIL_DOMAIN}{reverse('get-photo', args=(image.id,))}"),
                                       "link": f"https://{settings.EMAIL_DOMAIN}{reverse('item-details', args=(item.id,))}"
                                       }, items))
        icon = f"https://{settings.EMAIL_DOMAIN}/static/itemManagement/crest.png"
    ...
    def sendPasswordResetEmail(user, token, uidb64):
        EmailManager.__throwIfEmailIsntConfigured()
        icon = f"https://{settings.EMAIL_DOMAIN}/static/itemManagement/crest.png"
        link = f"https://{settings.EMAIL_DOMAIN}{reverse('user-register', args=(uidb64, token,))}"
